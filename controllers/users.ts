...
const signin = async (req: Request, res: Response) => {
    ...
    try {
        ...
        const token = jwt.sign({ email: existingUser.email, id: existingUser._id }, process.env.SECRET_KEY, {expiresIn: "10m"});
...
const signup = async (req: Request, res: Response) => {
    ...
    try {
        ...
        try {
            const token = jwt.sign({ email: result.email, id: result._id }, process.env.SECRET_KEY, { expiresIn: "10m" });
